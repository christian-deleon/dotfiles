...
3. **App configs picker** — gum multi-select of auto-discovered stow packages + tmux + ecc (sorted alphabetically with descriptions)
...
- `ecc` — Everything Claude Code (git submodule fork at `~/.dotfiles/ecc/`)
...
| ecc | `ecc` | dotfiles | Everything Claude Code (forked, curated branch) |
...
dot update                            # Update system packages, dotfiles, submodules, and ECC symlinks
...
[Everything Claude Code](https://github.com/affaan-m/everything-claude-code) is a forked git submodule at `~/.dotfiles/ecc/` (branch: `dotfiles`). The `dotfiles` branch is a curated subset — `main` stays as a clean mirror of upstream for easy syncing.

**What `install_ecc()` does:**

- **Claude Code** (`~/.claude/`):
  - Rules, commands, skills, agents — per-item symlinks via `link_directory_contents` (coexists with personal files)
...
cd ~/.dotfiles/ecc
git fetch upstream
git merge upstream/main    # on the dotfiles branch
```

**Removing ECC:** Delete the submodule, remove the `ecc` case from `install_app_config`, and clean up symlinks in `~/.claude/` and `~/.config/opencode/commands/`.
...
### Fork + Branch Strategy for Third-Party Submodules

When adding third-party config repos (like ECC), fork the project and use a dedicated branch (e.g., `dotfiles`). Keep `main` as a clean upstream mirror for easy syncing. The named branch holds your curated subset — deletions and customizations. This avoids merge conflicts when syncing upstream because your changes are isolated from the upstream history.

### Symlink Strategy: Merge vs Replace

Use `link_file` (symlinks the entire directory) when the directory is exclusively owned by one source. Use `link_directory_contents` (symlinks each item inside) when personal files need to coexist alongside sourced files. Example: `~/.claude/commands/` uses `link_directory_contents` so personal commands like `handoff.md` coexist with ECC commands.
...
3. **App configs**: `list_app_configs()` auto-discovers stow packages + tmux + ecc; `install_app_config()` handles each; `get_app_label()` provides descriptions for the picker
...
6. **Idempotency**: All modules must be safe to re-run. Use `ln -snf` for symlinks, check before stowing, skip if already done. ECC uses `clean_ecc_symlinks()` to remove stale links before re-linking
...
3. `dot update` handles OS-appropriate package updates (pacman/yay, apt, brew), pulls dotfiles, updates all submodules, and re-runs `install_ecc` if the ECC submodule exists
