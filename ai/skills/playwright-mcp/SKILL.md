...
Walker: **Chromium Agent**. Windows use class `chromium-agent` and Hyprland pops them (float + pin, 1300×1500) so they overlay instead of retileing the workspace. Playwright launches with `--sandbox` so Chromium does not show the unsupported `--no-sandbox` infobar.
