---
name: aws-mcp
description: AWS API access via the `aws-api` MCP server. Use whenever a task requires reading live AWS state (S3, EC2, IAM, Lambda, CloudWatch, RDS, EKS) or state-of-the-cloud questions — 'what's in this bucket', 'who has access to X', 'list my instances', 'show me the policy'. Read-only by default.
